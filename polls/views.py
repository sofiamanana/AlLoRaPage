...
def getNodos(request):
    ...
    response = requests.get('http://servicio1:'+ puerto +'/api/nodes/')
    data = response
    ...
    if response.status_code == 200:
        ...
        if nodo:
            ...
            if active:
                response = requests.post('http://servicio1:'+ puerto +'/api/setActiveNode/',data={'nodo': nodo}) 
                data = response
                if response.status_code == 200: 
                    data = response.json()
                    rows = functions.rowsToNodo(data)
            ...
            if mesh:
                response = requests.post('http://servicio1:'+ puerto +'/api/setMeshNode/',data={'nodo': nodo}) 
                data = response
                if response.status_code == 200:
                    data = response.json()
                    rows = functions.rowsToNodo(data)
                    return redirect('getNodos')
            if erase:
                response = requests.post('http://servicio1:'+ puerto +'/api/deleteNode/',data={'nodo': nodo}) 
                data = response
                if response.status_code == 200:
                    data = response.json()
                    rows = functions.rowsToNodo(data)
            ...
            if download:
                ...
                response = requests.get('http://servicio1:'+ puerto +'/api/downloadDataNode/',data={'node': nodo})
        ...
        if download_all:
            ...
            response = requests.get('http://servicio1:'+ puerto +'/api/downloadAll/')
            data = response.json()
            nombre_archivo = "data.json"
            cur_path = settings.BASE_DIR
    ...
    if request.method == 'POST':
        ...
        response = requests.get('http://servicio1:'+ puerto +'/api/getNode/',data={'mac_address': mac_address}) 
        data = response.json()
        print('entre2')
        print(data)
        print(nodo)
        if response.status_code == 200:
            if data["node"]:
                response = requests.post('http://servicio1:'+ puerto +'/api/updateNode/',data=nodo) 
                data = response
                print('entre3')
                if response.status_code == 200:
                    data = response.json()
                ...
                response = requests.post('http://servicio1:'+ puerto +'/api/addNode/',data=nodo) 
                data = response
                if response.status_code == 200:
                    data = response.json()
                    rows = functions.rowsToNodo(data)
...
def getGateway(request):
    if request.method == 'GET':
        ...
        response = requests.get('http://servicio1:'+ puerto +'/api/gateway/')
        data = response

        if response.status_code == 200:
            response2 = requests.get('http://servicio1:'+ puerto +'/api/getState/')
        ...
        print(activateg)
        if activateg:
            response = requests.get('http://servicio1:'+ puerto +'/api/activateg/')
            data = response
            return redirect('getGateway')
        stop = request.GET.get('stop')
        if stop:
            response = requests.get('http://servicio1:'+ puerto +'/api/deactivateg/')
            data = response
            return redirect('getGateway')

        reload = request.GET.get('reload')
        if reload:
            response = requests.get('http://servicio1:'+ puerto +'/api/getState/')
            state = response.json()["state"]
            if state:

                response = requests.get('http://servicio1:'+ puerto +'/api/restartGateway/')
